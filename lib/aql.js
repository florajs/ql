...
var Aql = {
    ...
    checkForSyntaxErrors: function checkForSyntaxErrors(query) {
        ...
        assert((pos = query.search(/\{[\-0-9\>\<\=a-zA-Z\.]*[\!\"\§\$\%\/\?\\\`\'\-\;\:\,\@]+[0-9\>\<\=a-zA-Z\.]*\}/)) === -1, error('Invalid character'));
    ...
    extractExpressions: function extractExpressions(term) {
        var exp = /[a-zA-Z\.]+\:\-?[0-9]+|[\&\|\(]\-?[0-9]+|[a-zA-Z\.]*\:?\{[\-0-9\<\>\=]+\}/;
        var ret = {};
        var key = 0;
        var prefix;
        var scope;
        while ((scope = term.match(exp)) !== null) {
            prefix = '';
            if (scope[0].substr(0,1) === '(' ||
                scope[0].substr(0,1) === '|' ||
                scope[0].substr(0,1) === '&') {
                prefix = scope[0].substr(0,1);
                scope[0] = scope[0].substr(1);
            }
            ++key;
            ret['e'+key] = scope[0];
            term = term.replace(prefix+scope[0], prefix+'e'+key);
        }
