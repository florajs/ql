...
var Aql = {
    ...
    checkForSyntaxErrors: function checkForSyntaxErrors(query) {
        ...
        // allowed type chars: [a-zA-ZäöüßÄÖÜ0-9\.]
        // allowed expression chars: [a-zA-ZäöüßÄÖÜ0-9\>\<\=\-\+\*\;\_\,~]
        ...
        assert((pos = query.search(/[^a-zA-ZäöüßÄÖÜ0-9]\[/)) === -1, error('Invalid subtyping'));
        ...
        assert((pos = query.search(/:[0-9]+,[0-9]+\}/)) === -1, error('Invalid set expression'));

        assert((pos = query.search(/\{[\-0-9a-zA-ZäöüßÄÖÜ\.\+\*;_]*\,+[\-0-9a-zA-ZäöüßÄÖÜ\.\+\*;_]*[><=~]+[\-0-9a-zA-ZäöüßÄÖÜ\.\+\*;_]*\}/)) === -1, error('Sets and operators cannot be mixed'));
        assert((pos = query.search(/\{[\-0-9a-zA-ZäöüßÄÖÜ\.\+\*;_,]+[><=~]+\}/)) === -1, error('Invalid expression'));
        assert((pos = query.search(/\{\}/)) === -1, error('Invalid expression'));
        regexp = /\{[a-zA-ZäöüßÄÖÜ0-9><=~\-\+\*;_,]*[:%\$&'\\!"\?\(\)#\|&_][a-zA-ZäöüßÄÖÜ0-9><=~\-\+\*;_:,]*\}/;
        assert((pos = query.search(regexp)) === -1, error('Invalid character in expression'));

        assert((pos = query.search(/\}[a-zA-ZäöüßÄÖÜ0-9]/)) === -1, error('Missing logical operator'));
        ...
        assert((pos = query.search(/[a-zA-ZäöüßÄÖÜ0-9\.]+\:/)) !== -1, error('At least one type is required'));

        regexp = new RegExp('\\}[\\&\\|](?![\\{\\(a-zA-ZäöüßÄÖÜ0-9].).');
    ...
    clearSubtypes: function clearSubtypes(query) {
        ...
        query = query.replace(/\.(?=[a-zA-ZäöüßÄÖÜ0-9\[\(\.]*:)/g, '#');
        log>=2 && console.log(query);

        var scope, term;
        while(scope = query.match(/#?([a-zA-ZäöüßÄÖÜ0-9#]+)\[([a-zA-ZäöüßÄÖÜ0-9\{\}\(\):\|&\-><=~;,\.\+\*_#]+)\]/)) {
            log>=2 && console.log(scope);
            term = scope[2].replace(/([a-zA-ZäöüßÄÖÜ0-9#]+):/g, scope[1]+'#$1:');
    ...
    extractExpressions: function extractExpressions(term) {
        ...
        var exp = /[a-zA-ZäöüßÄÖÜ0-9#]+:\-?[0-9a-zA-ZäöüßÄÖÜ]+|[&\|\(]\-?[0-9]+|[&\|\(][a-zA-ZäöüßÄÖÜ]+[&\|\)@]|[a-zA-ZäöüßÄÖÜ0-9#]*:?\{[\-:0-9a-zA-ZäöüßÄÖÜ;,.<>=~]+\}/;
    ...
    parse: function parse(query, options) {
        ...
        log && console.log('replacePresets', query);
        query = Aql.replacePresets(query);
        log && console.log('replaceSets', query);
        query = Aql.replaceSets(query);
        log && console.log('trim', query);
        query = Aql.trim(query);
        log && console.log('checkForSyntaxErrors', query);
        Aql.checkForSyntaxErrors(query);
        log && console.log('clearSubtypes', query);
        query = Aql.clearSubtypes(query);
        log && console.log('extractExpressions', query);
        query = Aql.extractExpressions(query);

        log && console.log('simplify', query);
        query[0] = Aql.simplify(query[0], '|', '&').split('|');
        log && console.log('setMissingTypes', query);
    ...
    replacePresets: function replacePresets(query) {
        ...
        while((preset = query.match(/([a-zA-ZäöüßÄÖÜ\.]+)\(([a-zA-ZäöüßÄÖÜ0-9\,]*)\)/)) !== null) {
            args = preset[2].split(',');
            if (args[0] === '') {
                args = [];
            }
    ...
    resolveDisjunctions: function resolveDisjunctions(query) {
        var scope, regex = /([a-zA-ZäöüßÄÖÜ0-9\.]+)\:(\{[a-zA-ZäöüßÄÖÜ0-9\-]*\,[a-zA-ZäöüßÄÖÜ0-9\,\-]*\})/;
        while ((scope = query.match(regex)) !== null) {
    ...
    simplify: function simplify(term, plus, mul) {
        ...
        log>=2 && console.log(regex);
        while((scope = term.match(regex)) !== null) {
            log && console.log(term);
            ...
            log && console.log('2-', term);
            
            // (a*b+a*c) -> a*b+a*c
            regexB = new RegExp('^\\(([\\w'+plusEx+mulEx+']+)\\)$');
            log>=2 && console.log('3-', regexB);
            if ((scopeB = term.match(regexB)) !== null) {
                term = term.replace(scopeB[0], scopeB[1]);
            }
            log && console.log('3-', term);
            
            // x+(a*b+a*c) -> x+a*b+a*c
            regexB = new RegExp('('+plusEx+')\\(([\\w'+plusEx+mulEx+']+)\\)$');
            log>=2 && console.log('4-', regexB);
            if ((scopeB = term.match(regexB)) !== null) {
                term = term.replace(scopeB[0], scopeB[1]+scopeB[2]);
            }
            log && console.log('4-', term);
            ...
            // (a*b+a*c)+x -> a*b+a*c+x
            regexB = new RegExp('^\\(([\\w'+plusEx+mulEx+']+)\\)('+plusEx+')');
            log>=2 && console.log('5-', regexB);
            if ((scopeB = term.match(regexB)) !== null) {
                term = term.replace(scopeB[0], scopeB[1]+scopeB[2]);
            }
            log && console.log('5-', term);
            ...
            // x+(a*b+a*c)+y -> x+a*b+a*c+y
            regexB = new RegExp(plusEx+'\\(([\\w'+plusEx+mulEx+']+)\\)'+plusEx);
            log>=2 && console.log('5-', regexB);
            if ((scopeB = term.match(regexB)) !== null) {
                term = term.replace(scopeB[0], plus+scopeB[2]+plus);
            }
            log && console.log('5-', term);
            
            // ((a*b+a*c)) -> (a*b+a*c)
            regexB = new RegExp('\\((\\([\\w'+plusEx+mulEx+']+\\))\\)');
            log>=2 && console.log('6-', regexB);
            if ((scopeB = term.match(regexB)) !== null) {
                term = term.replace(scopeB[0], scopeB[1]);
            }
            log && console.log('6-', term);
        }
    ...
    setMissingTypes: function setType(query, mul) {
        ...
        for (var i=0, len=query[0].length; i<len; ++i) {
            ...
            for (var j=0, lenj=exp.length; j<lenj; ++j) {
                currentType = query[1][exp[j]].match(/([a-zA-ZäöüßÄÖÜ0-9\.\#]+)\:(.+)/);
