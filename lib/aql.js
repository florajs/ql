...
var Aql = {
    ...
    checkForSyntaxErrors: function checkForSyntaxErrors(query) {
        ...
        assert((pos = query.search(/[\!\"\§\$\%\/\?\\\`\'\,]/)) === -1, error('Invalid character'));
        ...
        assert((pos = query.search(/[a-zA-Z\.]\:\[\]/)) === -1, error('Invalid subtyping'));
        assert((pos = query.search(/[^a-zA-Z]\[/)) === -1, error('Invalid subtyping'));
        assert((pos = query.search(/\{[\-0-9a-zA-Z\.]+[\>\<\=]+\}/)) === -1, error('Invalid expression'));
        //assert((pos = query.search(/\{[\>\<\=]+[\-0-9a-zA-Z\.]+\}/)) === -1, error('Invalid expression'));
        assert((pos = query.search(/\{[\-0-9a-zA-Z\.]+\}/)) === -1, error('Invalid expression'));
        assert((pos = query.search(/[0-9][a-zA-Z]/)) === -1, error('Missing logical operator'));
        assert((pos = query.search(/[a-zA-Z\.]+\:\(/)) === -1, error('Type must be followed by value or expression'));

        regexp = new RegExp('\\{[\-0-9a-zA-Z\.\\>\\<\\=]*[\\&\\|\\(\\)]+[\-0-9a-zA-Z\.\\>\\<\\=]*\\}');
        assert((pos = query.match(regexp)) === null, error('Invalid expression: "'+(pos!==null? pos[0] : '')+'"'));
        ...
        assert((pos = query.search(/[a-zA-Z\.]+\:/)) !== -1, error('At least one type is required'));
    ...
    clearSubtypes: function clearSubtypes(query) {
        ...
        var scope, term;
        while(scope = query.match(/([a-zA-Z\.]+)\[([a-zA-Z0-9\{\}\(\)\:\|\&\-\>\<\=\;\+\#]+)\]/)) {
            term = scope[2].replace(/([a-zA-Z\#]+)\:/g, scope[1]+'#$1:');
    ...
    extractExpressions: function extractExpressions(term) {
        ...
        var exp = /[a-zA-Z\#]+\:\-?[0-9a-zA-Z]+|[\&\|\(]\-?[0-9]+|[\&\|\(][a-zA-Z]+[\&\|\)\@]|[a-zA-Z\#]*\:?\{[\-0-9\<\>\=]+\}/;
    ...
    parse: function parse(query) {
        ...
        for (var i=0, l=query[0].length; i<l; ++i) {
            ...
            for (var j=0, lj=query[0][i].length; j<lj; ++j) {
                ...
                if (!(exp[0] in ret[ret.length-1])) {
                    ret[ret.length-1][exp[0]] = [];
                }
                ret[ret.length-1][exp[0]].push(exp[1]);
            }
        }
        return ret;
    },
    ...
    setType: function setType(query, mul) {
        ...
        for (var i=0, len=query[0].length; i<len; ++i) {
            ...
            for (var j=0, lenj=exp.length; j<lenj; ++j) {
                currentType = query[1][exp[j]].split(':');
                type = currentType.length === 2? currentType[0] : type;

                if (j === 0 && type === null) {
                    throw new Error('Missing required type at "'+query[1][exp[j]]+'"');
                }

                query[1][exp[j]] = type + ':' +(currentType.length === 2? currentType[1] : currentType[0]);
            }
        }
        return query;
    },
